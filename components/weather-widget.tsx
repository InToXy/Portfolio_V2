...
export function WeatherWidget() {
  ...
  useEffect(() => {
    ...
    const fetchWeatherData = async () => {
      ...
      try {
        ...
        const weatherInfo = weatherCodeMap[data.current_weather.weathercode] || {
          condition: 'Inconnu',
          icon: '04d',
        }
  ...
  const getWeatherIcon = (icon: string) => {
    switch (icon) {
      ...
      case '50d':
      case '50n':
        return <Cloud className="h-6 w-6 text-gray-400" /> // Fog
      case '11d':
      case '11n':
        return <CloudRain className="h-6 w-6 text-gray-600" /> // Thunderstorm (using rain icon for simplicity)
      default:
        return <Cloud className="h-6 w-6 text-gray-500" />
    }
  }
  ...
  return (
    <Card className="bg-background/80 border-primary/20 animate-in slide-in-from-top-4 fixed top-4 right-28 z-50 backdrop-blur-md delay-500 duration-1000">
      <CardContent className="space-y-3 p-4">
        {/* Date and Time */}
        <div className="text-center">
          <div className="text-primary animate-pulse font-mono text-2xl font-bold">
        ...
        {/* Weather */}
        {loading ? (
          <div className="text-muted-foreground mt-2 text-center text-xs">Chargement météo...</div>
        ) : weather ? (
